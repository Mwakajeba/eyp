                                ...
                                <div class="row mb-4">
                                    ...
                                    <div class="col-lg-4">
                                        <div class="mb-3">
                                            ...
                                            <select class="form-select form-select-lg select2-single @error('project_id') is-invalid @enderror"
                                                id="project_id" name="project_id">
                                                <option value="">None</option>
                                                @foreach($projects as $project)
                                                    <option value="{{ $project->id }}" {{ old('project_id', $paymentVoucher->project_id) == $project->id ? 'selected' : '' }}>
                                                        {{ $project->project_code ? $project->project_code . ' - ' : '' }}{{ $project->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                            <small class="text-muted">Optional: link this payment voucher to a project.</small>
                                            @error('project_id')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>

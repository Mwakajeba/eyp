...
<div class="page-wrapper">
    <div class="page-content">
        <x-breadcrumbs-with-icons :links="[
            ...
            ['label' => 'Assets by Location', 'url' => '#', 'icon' => 'bx bx-map']
        ]" />

        <div class="row">
            <div class="col-12">
                <div class="page-title-box d-flex align-items-center justify-content-between">
                    <h4 class="mb-0">Assets by Location Report</h4>
                ...
                <div class="card">
                    <div class="card-header bg-info text-white">
                        <h5 class="card-title mb-0">
                            <i class="bx bx-map me-2"></i>Assets by Location Report
                        </h5>
                    </div>
                    <div class="card-body">
                        <!-- Summary Cards -->
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered table-sm table-hover" id="by-location-table">
...
$(document).ready(function() {
    ...
    function generateReport() {
        ...
        $.ajax({
            url: '{{ route('assets.reports.by-location.data') }}',
            type: 'GET',
            data: {
    ...
    function renderReport(data, summary) {
        ...
        // Render rows grouped by location
        Object.keys(groupedData).sort().forEach(function(location) {
            let locationAssets = groupedData[location];
            let locationCost = 0;
            let locationNbv = 0;

            locationAssets.forEach(function(asset, index) {
                html += '<tr>';
                if (index === 0) {
                    html += '<td rowspan="' + locationAssets.length + '" class="bg-light fw-bold">' + location + '</td>';
                }
                html += '<td>' + asset.asset_code + '</td>';
                html += '<td>' + asset.asset_name + '</td>';
                html += '<td>' + asset.category + '</td>';
                html += '<td class="text-end">' + formatCurrency(asset.cost) + '</td>';
                html += '<td class="text-end">' + formatCurrency(asset.nbv) + '</td>';
                html += '</tr>';

                locationCost += parseFloat(asset.cost);
                locationNbv += parseFloat(asset.nbv);
            });

            // Add subtotal for location
            html += '<tr class="table-secondary fw-bold">';
            html += '<td colspan="4" class="text-end">Subtotal - ' + location + '</td>';
            html += '<td class="text-end">' + formatCurrency(locationCost) + '</td>';
            html += '<td class="text-end">' + formatCurrency(locationNbv) + '</td>';
            html += '</tr>';
        });

        // Add grand total
        html += '<tr class="table-warning fw-bold">';
        html += '<td colspan="4" class="text-end">GRAND TOTAL</td>';
        html += '<td class="text-end">' + formatCurrency(summary.total_cost) + '</td>';
        html += '<td class="text-end">' + formatCurrency(summary.total_nbv) + '</td>';
        html += '</tr>';

        if (data.length === 0) {
            html = '<tr><td colspan="6" class="text-center text-muted">No assets found.</td></tr>';
        }

        $('#by-location-tbody').html(html);
    }
    ...
    function exportToExcel() {
        ...
        const form = $('<form>', {
            'method': 'POST',
            'action': '{{ route('assets.reports.by-location.export-excel') }}'
        });
    ...
    function exportToPdf() {
        ...
        const form = $('<form>', {
            'method': 'POST',
            'action': '{{ route('assets.reports.by-location.export-pdf') }}'
        });

<form action="{{ isset($bankAccount) ? route('accounting.bank-accounts.update', \Vinkla\Hashids\Facades\Hashids::encode($bankAccount->id)) : route('accounting.bank-accounts.store') }}" method="POST" id="bank-account-form">
    ...
    <div class="mb-4">
        ...
        <div class="row g-3">
            ...
            <div class="col-md-6">
                <div class="form-group">
                    <label for="account_nature" class="form-label fw-semibold">
                        <i class="bx bx-category me-1 text-primary"></i>
                        Account Nature <span class="text-danger">*</span>
                    </label>
                    <select class="form-select @error('account_nature') is-invalid @enderror" name="account_nature" id="account_nature" required>
                        <option value="bank" {{ (old('account_nature', $bankAccount->account_nature ?? 'bank') == 'bank') ? 'selected' : '' }}>Bank</option>
                        <option value="card" {{ (old('account_nature', $bankAccount->account_nature ?? '') == 'card') ? 'selected' : '' }}>Card (e.g. Fuel Card)</option>
                    </select>
                    <small class="form-text text-muted">
                        <i class="bx bx-info-circle me-1"></i>
                        Use &quot;Card&quot; for fuel cards or payment cards that can be assigned to drivers.
                    </small>

...
@section('title', 'Driver Collection Report')
...
<div class="page-wrapper">
    <div class="page-content">
        <x-breadcrumbs-with-icons :links="[
            ...
            ['label' => 'Driver Collection Report', 'url' => '#', 'icon' => 'bx bx-wallet']
        ]" />

        <h6 class="mb-0 text-uppercase">DRIVER COLLECTION REPORT</h6>
        <hr />

        <!-- Filters -->
        <div class="card">
            <div class="card-body">
                <form method="GET" action="{{ route('fleet.reports.driver-collection') }}" class="row g-3">
                    <div class="col-md-5">
        ...
        <div class="card">
            <div class="card-body">
                <div class="d-flex justify-content-end gap-2">
                    <form method="POST" action="{{ route('fleet.reports.driver-collection.export-excel') }}" style="display: inline;">
                        @csrf
                    ...
                    <form method="POST" action="{{ route('fleet.reports.driver-collection.export-pdf') }}" style="display: inline;">
                        @csrf
                        <input type="hidden" name="date_from" value="{{ request('date_from') }}">
                        <input type="hidden" name="date_to" value="{{ request('date_to') }}">
                        ...
                        <tbody>
                            @forelse($collectionData as $data)
                                <tr>
                                    ...
                                    <td class="text-end"><strong>{{ number_format($data['revenue_collected'], 2) }}</strong></td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="4" class="text-center">No collection data found for the selected criteria.</td>
                                </tr>
                            @endforelse
                        </tbody>
                        <tfoot>
                            <tr>
                                ...
                                <th class="text-center">{{ $collectionData->sum('trips') }}</th>

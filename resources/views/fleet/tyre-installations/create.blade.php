...
@section('title', 'Install Tyre - Fleet Management')
...
<div class="page-wrapper">
    <div class="page-content">
        <x-breadcrumbs-with-icons :links="[
            ...
            ['label' => 'Add Installation', 'url' => '#', 'icon' => 'bx bx-plus']
        ]" />

        <div class="card">
            <div class="card-header bg-success text-white">
                <h6 class="mb-0"><i class="bx bx-plus me-2"></i>Install Tyre on Vehicle</h6>
            </div>
            <div class="card-body">
                <form method="POST" action="{{ route('fleet.tyre-installations.store') }}">
                    @csrf
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <select name="tyre_id" class="form-select select2-single" required>
                                ...
                                @foreach($tyres as $t)
                                    <option value="{{ $t->id }}" {{ old('tyre_id') == $t->id ? 'selected' : '' }}>{{ $t->tyre_serial }} — {{ $t->brand ?? 'N/A' }} ({{ $t->status }})</option>
                                @endforeach
                            </select>
                            @if($tyres->isEmpty())
                                <small class="text-muted">No tyres available (register tyres with status New or Removed first).</small>
                            @endif
                        ...
                        <div class="col-md-6">
                            ...
                            @if($positions->isEmpty())
                                <small class="text-muted">No positions defined. Add positions in Truck Tyre Configuration first.</small>
                            @endif
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="number" name="odometer_at_install" class="form-control" value="{{ old('odometer_at_install') }}" min="0" step="0.01" placeholder="km">
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="installer_type" class="form-select select2-single">
                                <option value="">—</option>
                                <option value="garage" {{ old('installer_type') === 'garage' ? 'selected' : '' }}>Garage</option>
                                <option value="internal" {{ old('installer_type') === 'internal' ? 'selected' : '' }}>Internal</option>
                            </select>
                            @error('installer_type')<div class="text-danger small">{{ $message }}</div>@enderror
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Installer name</label>
                            <input type="text" name="installer_name" class="form-control" value="{{ old('installer_name') }}" maxlength="255" placeholder="Garage or mechanic name">
                        ...
                        <div class="col-12">
                            <button type="submit" class="btn btn-success"><i class="bx bx-save me-1"></i>Record Installation</button>
                            <a href="{{ route('fleet.tyre-installations.index') }}" class="btn btn-secondary">Cancel</a>
                        </div>
                    </div>
                </form>
            </div>

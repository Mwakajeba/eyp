...
<div class="page-wrapper">
    <div class="page-content">
        <x-breadcrumbs-with-icons :links="[
            ...
            ['label' => 'Create Proposal', 'url' => '#', 'icon' => 'bx bx-plus']
        ]" />
        
        <h6 class="mb-0 text-uppercase">CREATE INVESTMENT PROPOSAL</h6>
        <hr />
        ...
        <form action="{{ route('investments.proposals.store') }}" method="POST" id="proposal-form">
            @csrf
            ...
            <div class="row">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select name="instrument_type" id="instrument_type" class="form-select select2-single" required>
                                        ...
                                        <option value="T_BILL" {{ old('instrument_type') == 'T_BILL' ? 'selected' : '' }}>T-Bill (Treasury Bill)</option>
                                        <option value="T_BOND" {{ old('instrument_type') == 'T_BOND' ? 'selected' : '' }}>T-Bond (Treasury Bond)</option>
                                        <option value="FIXED_DEPOSIT" {{ old('instrument_type') == 'FIXED_DEPOSIT' ? 'selected' : '' }}>Fixed Deposit</option>
                                        <option value="CORP_BOND" {{ old('instrument_type') == 'CORP_BOND' ? 'selected' : '' }}>Corporate Bond</option>
                                        <option value="EQUITY" {{ old('instrument_type') == 'EQUITY' ? 'selected' : '' }}>Equity (Stock/Share)</option>
                                        <option value="MMF" {{ old('instrument_type') == 'MMF' ? 'selected' : '' }}>Money Market Fund</option>
                                        <option value="OTHER" {{ old('instrument_type') == 'OTHER' ? 'selected' : '' }}>Other</option>
                                    </select>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" name="issuer" id="issuer" class="form-control" value="{{ old('issuer') }}" placeholder="e.g., Bank of Tanzania, ABC Corporation">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="number" name="proposed_amount" id="proposed_amount" class="form-control" step="0.01" min="0.01" value="{{ old('proposed_amount') }}" required placeholder="0.00">
                                    </div>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <div class="input-group">
                                        <input type="number" name="expected_yield" id="expected_yield" class="form-control" step="0.01" min="0" max="100" value="{{ old('expected_yield') }}" placeholder="0.00">
                                ...
                                <div class="col-md-4 mb-3">
                                    ...
                                    <select name="risk_rating" id="risk_rating" class="form-select select2-single">
                                        ...
                                        <option value="LOW" {{ old('risk_rating') == 'LOW' ? 'selected' : '' }}>Low Risk</option>
                                        <option value="MEDIUM" {{ old('risk_rating') == 'MEDIUM' ? 'selected' : '' }}>Medium Risk</option>
                                        <option value="HIGH" {{ old('risk_rating') == 'HIGH' ? 'selected' : '' }}>High Risk</option>
                                    </select>
                                ...
                                <div class="col-md-4 mb-3">
                                    ...
                                    <div class="input-group">
                                        <input type="number" name="tenor_value" id="tenor_value" class="form-control" step="0.01" min="0.01" value="{{ old('tenor_value', old('tenor_days')) }}" placeholder="Enter value">
                                        <select name="tenor_unit" id="tenor_unit" class="form-select" style="max-width: 100px;">
                                            <option value="days">Days</option>
                                            <option value="years">Years</option>
                                        </select>
                                        <input type="hidden" name="tenor_days" id="tenor_days" value="{{ old('tenor_days') }}">
                                    </div>
                                ...
                                <div class="col-md-4 mb-3">
                                    ...
                                    <select name="proposed_accounting_class" id="proposed_accounting_class" class="form-select select2-single" required>
                                        <option value="AMORTISED_COST" {{ old('proposed_accounting_class', 'AMORTISED_COST') == 'AMORTISED_COST' ? 'selected' : '' }}>Amortized Cost</option>
                                        <option value="FVOCI" {{ old('proposed_accounting_class') == 'FVOCI' ? 'selected' : '' }}>FVOCI (Fair Value OCI)</option>
                                        <option value="FVPL" {{ old('proposed_accounting_class') == 'FVPL' ? 'selected' : '' }}>FVPL (Fair Value P&L)</option>
                                    </select>
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-12 mb-3">
                                    ...
                                    <textarea name="description" id="description" class="form-control" rows="4" placeholder="Describe the investment instrument, its features, and key terms...">{{ old('description') }}</textarea>
                                ...
                                <div class="col-md-12 mb-3">
                                    ...
                                    <textarea name="rationale" id="rationale" class="form-control" rows="5" placeholder="Explain why this investment is being proposed. Include investment objectives, expected benefits, risk considerations, and strategic alignment...">{{ old('rationale') }}</textarea>
                                    <small class="text-muted d-block mt-1">
                ...
                <div class="col-lg-4">
                    <div class="card sticky-top" style="top: 20px;">
                        <div class="card-header bg-primary bg-gradient text-white">
                            <h6 class="mb-0">
                                <i class="bx bx-info-circle me-2"></i>Proposal Creation Guide
                            </h6>
                        </div>
                        <div class="card-body">
                            <div class="alert alert-info mb-3">
                                ...
                                <ul class="mb-0 small">
                                    <li>Select appropriate instrument type</li>
                                    <li>Provide accurate financial details</li>
                                    <li>Include comprehensive rationale</li>
                                    <li>Risk rating helps approvers assess</li>
                                    <li>Proposal number is auto-generated</li>
                                </ul>
                            </div>
                            ...
                            <div class="alert alert-success mb-3">
                                <h6 class="alert-heading">
                                    <i class="bx bx-check-circle me-1"></i>Instrument Types
                                </h6>
                                <ul class="mb-0 small">
                                    <li><strong>T-Bill:</strong> Short-term government security</li>
                                    <li><strong>T-Bond:</strong> Long-term government bond</li>
                                    <li><strong>Fixed Deposit:</strong> Bank time deposit</li>
                                    <li><strong>Corporate Bond:</strong> Company-issued bond</li>
                                    <li><strong>Equity:</strong> Stock/share investment</li>
                                    <li><strong>MMF:</strong> Money market fund</li>
                                </ul>
                            </div>

                            <div class="alert alert-primary">
                                <h6 class="alert-heading">
                                    <i class="bx bx-book me-1"></i>Accounting Classes
                                </h6>
                                <ul class="mb-0 small">
                                    <li><strong>Amortized Cost:</strong> Held to maturity</li>
                                    <li><strong>FVOCI:</strong> Fair value through OCI</li>
                                    <li><strong>FVPL:</strong> Fair value through P&L</li>
                                </ul>
                            </div>

                            <div class="mt-3">
                                <h6 class="text-muted">Approval Workflow</h6>
                                <div class="bg-light p-2 rounded small">
                                    <strong>Status Flow:</strong><br>
                                    DRAFT → SUBMITTED →<br>
                                    IN_REVIEW → APPROVED<br>
                                    <small class="text-muted">Multi-level approval required</small>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <small class="text-muted">
                                ...
                                Proposal will be saved as DRAFT. Submit for approval when ready.
                            </small>
                        </div>
                        <div class="btn-group">
                            <a href="{{ route('investments.proposals.index') }}" class="btn btn-secondary">
                                <i class="bx bx-x"></i> Cancel
                            </a>
                            <button type="submit" class="btn btn-primary">
                                <i class="bx bx-save"></i> Save Proposal
                            </button>
                        </div>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function convertTenorToDays() {
        ...
        let days;
        if (unit === 'years') {
            days = Math.round(value * 365); // Simple conversion: 1 year = 365 days
        } else {
    ...
    // Set default unit when instrument type changes
    ...
    // Set initial default based on old value or instrument type
    if (instrumentTypeSelect && instrumentTypeSelect.value) {
        setDefaultTenorUnit();
    } else if (tenorValueInput && tenorValueInput.value) {
        // If old value exists, try to detect if it's in years or days
        const oldValue = parseFloat(tenorValueInput.value);
        if (oldValue && oldValue < 100 && tenorUnitSelect) {
            // Likely years
            tenorUnitSelect.value = 'years';
            convertTenorToDays();
        } else if (oldValue && tenorUnitSelect) {
            // Likely days
            tenorUnitSelect.value = 'days';
            convertTenorToDays();
        }
    ...
    // Convert on form submit to ensure latest value
    const proposalForm = document.getElementById('proposal-form');
    if (proposalForm) {
        proposalForm.addEventListener('submit', function(e) {
            if (tenorValueInput && tenorDaysHidden) {
                convertTenorToDays();
                if (!tenorDaysHidden.value) {
                    e.preventDefault();
                    alert('Please enter a valid tenor value.');
                    tenorValueInput.focus();
                    return false;
                }
            }
        });

	...
	<div class="page-content">
		...
		<div class="card shadow-sm">
			<div class="card-header bg-primary text-white">
				<h5 class="card-title mb-0"><i class="bx bx-plus me-2"></i>New Loan Facility</h5>
			</div>
			<div class="card-body">
				<form method="POST" action="{{ route('loans.store') }}" id="loanForm">
						...
						<div class="card-body">
							<div class="row g-3">
								<div class="col-md-4">
									...
									<input type="number" class="form-control" name="lender_id" value="{{ old('lender_id') }}" placeholder="e.g., 1001">
								...
								<div class="col-md-4">
									...
									<input type="text" class="form-control" name="lender_name" value="{{ old('lender_name') }}" placeholder="e.g., ABC Bank Ltd">
								...
								<div class="col-md-6">
									...
									<input type="text" class="form-control" name="bank_name" value="{{ old('bank_name') }}" placeholder="e.g., ABC Bank Ltd">
								...
								<div class="col-md-6">
									...
									<input type="text" class="form-control" name="bank_contact" value="{{ old('bank_contact') }}" placeholder="e.g., John Doe, +255 123 456 789">
						...
						<div class="card-body">
							<div class="row g-3">
								<div class="col-md-6">
									...
									<input type="text" class="form-control" name="facility_name" value="{{ old('facility_name') }}" placeholder="e.g., Working Capital Loan 2025">
								...
								<div class="col-md-6">
									...
									<select name="facility_type" class="form-select">
										...
										@foreach(['term_loan' => 'Term Loan', 'revolving' => 'Revolving', 'overdraft' => 'Overdraft', 'line_of_credit' => 'Line of Credit', 'other' => 'Other'] as $key => $label)
											<option value="{{ $key }}" {{ old('facility_type') == $key ? 'selected' : '' }}>{{ $label }}</option>
										@endforeach
									</select>
						...
						<div class="card-body">
							<div class="row g-3">
								<div class="col-md-4">
									...
									<input type="number" step="0.01" class="form-control" name="principal_amount" value="{{ old('principal_amount') }}" required placeholder="0.00">
								...
								<div class="col-md-4">
									...
									<input type="number" step="0.01" class="form-control" name="disbursed_amount" value="{{ old('disbursed_amount') }}" placeholder="Same as principal if not specified">
								...
								<div class="col-md-4">
									...
									<input type="date" class="form-control" name="disbursement_date" value="{{ old('disbursement_date', now()->format('Y-m-d')) }}">
								...
								<div class="col-md-4">
									...
									<input type="date" class="form-control" name="maturity_date" value="{{ old('maturity_date') }}">
								...
								<div class="col-md-4">
									...
									<input type="date" class="form-control" name="first_payment_date" value="{{ old('first_payment_date') }}">
						...
						<div class="card-body">
							<div class="row g-3">
								<div class="col-md-4">
									...
									<input type="number" step="0.01" class="form-control" name="interest_rate" value="{{ old('interest_rate') }}" required placeholder="e.g., 10.5">
								...
								<div class="col-md-4">
									...
									<select name="rate_type" class="form-select" id="rate_type" required>
										@foreach(['fixed' => 'Fixed', 'variable' => 'Variable'] as $key => $label)
											<option value="{{ $key }}" {{ old('rate_type','fixed') == $key ? 'selected' : '' }}>{{ $label }}</option>
										@endforeach
									</select>
								...
								<div class="col-md-4" id="base_rate_source_field" style="display: none;">
									<label class="form-label">
										Base Rate Source
									...
									<input type="text" class="form-control" name="base_rate_source" value="{{ old('base_rate_source') }}" placeholder="e.g., Treasury Bill Rate">
								...
								<div class="col-md-4" id="spread_field" style="display: none;">
									<label class="form-label">
										Spread (%)
										<i class="bx bx-info-circle text-muted ms-1" data-bs-toggle="tooltip" title="Additional percentage added to base rate"></i>
									</label>
									<input type="number" step="0.01" class="form-control" name="spread" value="{{ old('spread') }}" placeholder="e.g., 2.5">
								...
								<div class="col-md-4">
									...
									<select name="calculation_basis" class="form-select" required>
										@foreach(['30/360' => '30/360', 'actual/365' => 'Actual/365', 'actual/360' => 'Actual/360'] as $key => $label)
											<option value="{{ $key }}" {{ old('calculation_basis','actual/365') == $key ? 'selected' : '' }}>{{ $label }}</option>
										@endforeach
									</select>
						...
						<div class="card-body">
							<div class="row g-3">
								<div class="col-md-4">
									...
									<input type="number" class="form-control" name="term_months" value="{{ old('term_months') }}" required placeholder="e.g., 12">
								...
								<div class="col-md-4">
									...
									<select name="payment_frequency" class="form-select" required>
										@foreach(['monthly' => 'Monthly', 'quarterly' => 'Quarterly', 'semi-annual' => 'Semi-Annual', 'annual' => 'Annual'] as $key => $label)
											<option value="{{ $key }}" {{ old('payment_frequency','monthly') == $key ? 'selected' : '' }}>{{ $label }}</option>
										@endforeach
									</select>
								...
								<div class="col-md-4">
									...
									<select name="amortization_method" class="form-select" required id="amortization_method">
										@foreach([
											...
											<option value="{{ $key }}" {{ old('amortization_method','annuity') == $key ? 'selected' : '' }}>{{ $label }}</option>
										@endforeach
									</select>
								...
								<div class="col-md-4">
									...
									<select name="repayment_method" class="form-select" id="repayment_method">
										...
										@foreach([
											...
											<option value="{{ $key }}" {{ old('repayment_method') == $key ? 'selected' : '' }}>{{ $label }}</option>
										@endforeach
									</select>
								...
								<div class="col-md-4">
									...
									<input type="number" class="form-control" name="grace_period_months" value="{{ old('grace_period_months', 0) }}" placeholder="0">
						...
						<div class="card-body">
							<div class="row g-3">
								<div class="col-md-4">
									...
									<input type="number" step="0.01" class="form-control" name="fees_amount" value="{{ old('fees_amount', 0) }}" placeholder="0.00">
								...
								<div class="col-md-4">
									...
									<select name="capitalise_fees" class="form-select">
										<option value="0" {{ old('capitalise_fees', 0) == 0 ? 'selected' : '' }}>No (Expense Immediately)</option>
										<option value="1" {{ old('capitalise_fees') == 1 ? 'selected' : '' }}>Yes (Capitalize)</option>
									</select>
								...
								<div class="col-md-4">
									...
									<select name="capitalise_interest" class="form-select">
										<option value="0" {{ old('capitalise_interest', 0) == 0 ? 'selected' : '' }}>No (Expense Interest)</option>
										<option value="1" {{ old('capitalise_interest') == 1 ? 'selected' : '' }}>Yes (Capitalise Interest)</option>
									</select>
								...
								<div class="col-md-4">
									...
									<select name="prepayment_allowed" class="form-select">
										<option value="1" {{ old('prepayment_allowed', 1) == 1 ? 'selected' : '' }}>Yes</option>
										<option value="0" {{ old('prepayment_allowed') == 0 ? 'selected' : '' }}>No</option>
									</select>
								...
								<div class="col-md-4">
									...
									<input type="number" step="0.01" class="form-control" name="prepayment_penalty_rate" value="{{ old('prepayment_penalty_rate') }}" placeholder="e.g., 2.0">
							...
							<div class="row g-3">
								<div class="col-md-6">
									...
									<select name="loan_payable_account_id" class="form-select select2-single" data-placeholder="Select payable account">
										...
										@foreach($loanLiabilityAccounts as $acc)
											<option value="{{ $acc->id }}" {{ old('loan_payable_account_id') == $acc->id ? 'selected' : '' }}>{{ $acc->account_code }} - {{ $acc->account_name }}</option>
										@endforeach
									</select>
								...
								<div class="col-md-6">
									...
									<select name="interest_expense_account_id" class="form-select select2-single" data-placeholder="Select interest expense account">
										...
										@foreach($interestExpenseAccounts as $acc)
											<option value="{{ $acc->id }}" {{ old('interest_expense_account_id') == $acc->id ? 'selected' : '' }}>{{ $acc->account_code }} - {{ $acc->account_name }}</option>
										@endforeach
									</select>
								...
								<div class="col-md-6">
									...
									<select name="interest_payable_account_id" class="form-select select2-single" data-placeholder="Select interest payable account">
										...
										@foreach($interestPayableAccounts as $acc)
											<option value="{{ $acc->id }}" {{ old('interest_payable_account_id') == $acc->id ? 'selected' : '' }}>{{ $acc->account_code }} - {{ $acc->account_name }}</option>
										@endforeach
									</select>
								...
								<div class="col-md-6">
									...
									<select name="deferred_loan_costs_account_id" class="form-select select2-single" data-placeholder="Select deferred costs account">
										...
										@foreach($deferredCostAccounts as $acc)
											<option value="{{ $acc->id }}" {{ old('deferred_loan_costs_account_id') == $acc->id ? 'selected' : '' }}>{{ $acc->account_code }} - {{ $acc->account_name }}</option>
										@endforeach
									</select>
								...
								<div class="col-md-6">
									...
									<select name="capitalised_interest_account_id" class="form-select select2-single" data-placeholder="Select capitalised interest asset account">
										<option value="">Select account</option>
										@foreach($deferredCostAccounts as $acc)
											<option value="{{ $acc->id }}" {{ old('capitalised_interest_account_id') == $acc->id ? 'selected' : '' }}>{{ $acc->account_code }} - {{ $acc->account_name }}</option>
										@endforeach
									</select>
								...
								<div class="col-md-6">
									...
									<select name="bank_charges_account_id" class="form-select select2-single" data-placeholder="Select bank charges account">
										...
										@foreach($bankChargeAccounts as $acc)
											<option value="{{ $acc->id }}" {{ old('bank_charges_account_id') == $acc->id ? 'selected' : '' }}>{{ $acc->account_code }} - {{ $acc->account_name }}</option>
										@endforeach
									</select>
								...
								<div class="col-md-6">
									...
									<select name="loan_processing_fee_account_id" class="form-select select2-single" data-placeholder="Select loan processing fee account">
										...
										@foreach($loanProcessingFeeAccounts as $acc)
											<option value="{{ $acc->id }}" {{ old('loan_processing_fee_account_id') == $acc->id ? 'selected' : '' }}>
						...
						<div class="card-body">
							<div class="row g-3">
								<div class="col-md-12">
									...
									<textarea name="notes" class="form-control" rows="4" placeholder="Enter any additional notes, terms, or special conditions...">{{ old('notes') }}</textarea>
					...
					<div class="d-flex justify-content-between align-items-center">
						...
						<button type="submit" class="btn btn-primary btn-lg">
							<i class="bx bx-save me-1"></i>Save Loan
						</button>
					</div>
				</form>
			</div>
...
<script nonce="{{ $cspNonce ?? '' }}">
	$(function(){
		...
		function recalculateTermMonths() {
			...
			// If end day is after start day, count it as an extra month
		...
		// Auto-fill Bank Name from Lender Name on create (editable)
		...
		lenderInput.on('blur change', function() {
			...
			// Only set bank name if it's currently empty or matches old lender name

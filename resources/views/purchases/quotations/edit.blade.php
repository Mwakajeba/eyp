...
<div class="page-wrapper">
    <div class="page-content">
        ...
        <h6 class="mb-0 text-uppercase">EDIT PURCHASE QUOTATION</h6>
        <hr />

        <div class="card">
            <div class="card-header bg-primary text-white">
                <h5 class="mb-0"><i class="bx bx-edit me-2"></i>Edit Purchase Quotation - {{ $quotation->reference }}</h5>
            </div>
            <div class="card-body">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select select2-single" id="supplier_id" name="supplier_id" required>
                                    ...
                                    @foreach($suppliers as $supplier)
                                        <option value="{{ $supplier->id }}" {{ $quotation->supplier_id == $supplier->id ? 'selected' : '' }}>{{ $supplier->name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-3">
                            <div class="mb-3">
                                ...
                                <input type="date" class="form-control" id="start_date" name="start_date" 
                                       value="{{ $quotation->start_date->format('Y-m-d') }}" required>
                                <div class="invalid-feedback"></div>
                            </div>
                        </div>
                        ...
                        <div class="col-md-3">
                            <div class="mb-3">
                                ...
                                <input type="date" class="form-control" id="due_date" name="due_date" 
                                       value="{{ $quotation->due_date->format('Y-m-d') }}" required>
                                <div class="invalid-feedback"></div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="mb-3">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" id="is_request_for_quotation" name="is_request_for_quotation" 
                                           {{ $quotation->is_request_for_quotation ? 'checked' : '' }}>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select select2-single" id="project_id" name="project_id">
                                    ...
                                    @foreach($projects ?? [] as $project)
                                        <option value="{{ $project->id }}" @selected($quotation->project_id == $project->id)>
                        ...
                        <div class="col-md-4">
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="notes" name="notes" rows="4" 
                                          placeholder="Additional notes for this quotation...">{{ $quotation->notes ?? '' }}</textarea>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="mb-3">
                        ...
                        <div class="col-md-4">
                            <div class="mb-3">
                                ...
                                @if(!empty($quotation->attachment))
                                    <div class="mt-2">
                                        <a href="{{ asset('storage/' . $quotation->attachment) }}" target="_blank">
...
$(document).ready(function() {
    ...
    @if($quotation->quotationItems)
        @foreach($quotation->quotationItems as $item)
            items.push({
                ...
                subtotal: {{ ($item->total_amount - $item->tax_amount) }},
    ...
    function loadExistingItems() {
        ...
        items.forEach(item => {
            ...
            // Apply RFQ state to existing items
            if (isRFQ) {
    ...
    $('#add-item-btn').click(function() {
        ...
        // Calculate VAT and totals (subtotal is net)
        let vatAmount = 0;
        ...
        if (!isRFQ) {
            if (vatType === 'inclusive' && vatRate > 0) {
                ...
                net = gross; vatAmount = 0; totalAmount = gross;
            }
        } else {
            net = gross; vatAmount = 0; totalAmount = gross;
        }
    ...
    $('#quotation-form').submit(function(e) {
        ...
        // Submit form
        $.ajax({
            url: '{{ route("purchases.quotations.update", $quotation->id) }}',
            method: 'POST',
            data: formData,
            processData: false,
            contentType: false,
